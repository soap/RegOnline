 ...
 * @since       1.0
 */
class RegonlineModelRegistration extends JModelAdmin
{
	/**
	...
	public function getItem($pk = null)
	{
		if ($result = parent::getItem($pk)) {
			...
			if (intval($result->registered)) {
				$date = new JDate($result->registered);
				$date->setTimezone($tz);
				$result->registered = $date->toSQL(true);
			}
			else {
	 ...
	 * @since   1.0
	 */
	public function getTable($type = 'Registration', $prefix = 'RegonlineTable', $config = array())
	{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->alias)) {
			...
			$table->alias = JUserHelper::genRandomPassword(6);
		}
		...
		if (empty($table->id)) {
			...
			if (empty($table->ordering)) {
				...
				$query->from('#__regonline_registrations');
