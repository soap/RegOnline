 ...
 * @since       1.0
 */
class RegonlineModelCourse extends JModelAdmin
{
	/**
	 * Method to get the Course form.
	 *
	 ...
	 * @since   1.0
	 */
	public function getTable($type = 'Course', $prefix = 'RegonlineTable', $config = array())
	{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (empty($table->ordering)) {
				...
				$query->from('#__regonline_courses');
	...
	/**
	 * Prepare form before display to user
	 * Modify form attributes according to user permission
	 * @param JForm $form
	 * @param unknown_type $data
	 * @param string $group
	 */
	protected function prepareForm(JForm $form, $data, $group = 'content')
	{
		$user = JFactory::getUser();
		if (!($user->authorise('core.edit.state', 'com_regonline')) ) {
